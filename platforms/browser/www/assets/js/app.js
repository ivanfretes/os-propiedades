...
//            }
   ...
   /* $(".button-collapse").click(function(){
    ...
    function setDateToMilisecond(value){
        fecha = value.split("/");
        if(fecha.length == 3){
            d = new Date(fecha[2],parseInt(fecha[1])-parseInt(1),parseInt(fecha[0]));    
    ...
    function setNextMonth(value){
       fecha = value.split("/");
        if(fecha.length == 3){
            ...
            d = new Date(fecha[2],fecha[1],fecha[0]);    
...
function htmlLoteDescription(lote){
    ...
    hText += "<tr><td>Seña: </td><td class=\"text-right\" id=\"descriptionSena\">"+lote.moneda+" "+formatNumber(isNaN(lote.importesena) ? lote.importesena.toString() : "0")+"</td></tr>";
    ...
    if ((lote.giLoteestado.trim() == "Disponible" || lote.giLoteestado.trim() == "Recuperado") 
        ...
        //$("#botonReserva").html("<a href=\"#SolicitudVenta\" id=\"btnActiveReserva\" data-lote=\""+lote.idgiLote+"\" data-activates=\"mobile-demo\" ><i style=\"font-size:40px;\" class=\"material-icons\">turned_in</i></a>"); 
        ...
        $("#botonReserva").html('');
        //console.log(lote.giLoteestado.trim());
